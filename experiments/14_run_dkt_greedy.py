"""Run the ECS32A DKT + Greedy baseline from the existing pyKT checkpoint."""
...
OUTPUT = ROOT / "results" / "dkt_greedy"
...
def predict_sequences(model, rows: pd.DataFrame, idx_to_node: dict[int, int]) -> tuple[list[float], list[float], dict[int, list[float]]]:
    labels: list[float] = []
    ...
    with torch.no_grad():
        for _, row in rows.iterrows():
            ...
            for index, target_idx in enumerate(concepts[1:]):
                if target_idx < 0 or target_idx not in idx_to_node:
                    continue
...
def main() -> None:
    ...
    fallback = float(np.mean(probabilities))
    oracle = DKTOracle(
