"""Build the deterministic Task 13 summary from frozen artifacts."""
...
GREEDY_RESULTS = ROOT / "results" / "dkt_set_greedy"
LAO_RESULTS = ROOT / "results" / "dkt_set_lao"
OUTPUT_PATH = RESULTS / "task13_summary.json"
...
def _validated_records(
    ...
    *,
    ...
    if len(records) != len(targets) or len(scored) != len(targets):
        raise ValueError(f"Expected one {expected_method.value} record and score per target")
    ...
    if any(row["method"] != expected_method.value for row in scored):
        raise ValueError(f"Unexpected method in {scored_path}")
    ...
    for record in records:
        for key, expected in expected_metadata.items():
            if record.metadata.get(key) != expected:
                raise ValueError(f"{expected_method.value} metadata mismatch for {key}")
...
def build_summary() -> dict:
    ...
    expected_metadata = {
        ...
        "surrogate_config_hash": config_hash,
    ...
    lao_records, lao_scored = _validated_records(
        ...
        expected_method=Method.DKT_SET_LAO,
        targets=targets,
        expected_metadata=expected_metadata,
    )
    ...
    if any(row["converged"] != "True" for row in comparisons):
        raise ValueError("A DKT-set LAO* run did not converge")
    lao_dp_max_gap = max(float(row["lao_dp_absolute_gap"]) for row in comparisons)
    max_lao_minus_greedy = max(
        float(row["lao_internal_cost"]) - float(row["greedy_internal_cost"])
    ...
    return {
        ...
        "lao_improved_targets": sum(float(row["greedy_minus_lao"]) > 1e-9 for row in comparisons),
        ...
        "generation_command": "python experiments/summarize_dkt_set.py",
    }
